...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .search-bar .left, .search-bar .right { display: flex; align-items: center; gap: 8px; }
    .search-bar .center { flex: 1; display: flex; align-items: center; justify-content: center; position: relative; gap: 20px; }
    .home-btn {
      display: inline-flex; align-items: center; gap: 6px;
      padding: 8px 14px; border-radius: 8px; text-decoration: none;
      color: #111; font-weight: 600; font-size: 15px; transition: .2s;
    }
    .home-btn:hover { background: #f9f6ee; color: #b8860b; }
    .header-logo { height: 52px; object-fit: contain; }
    .search-box {
      display: flex; align-items: center; gap: 6px;
      background: #f5f5f5; border-radius: 999px; padding: 6px 10px;
      border: 1px solid #e0e0e0; min-width: 260px; flex: 0 1 420px;
    }
    .search-box input { flex: 1; border: 0; outline: 0; background: transparent; font-size: 14px; }
    .search-box button {
      background: #b8860b; color: #fff; border: 0; border-radius: 999px;
      padding: 7px 13px; cursor: pointer; font-size: 13px; transition: .2s;
    }
    .search-box button:hover { background: #8b6408; }
    .icon-link {
      display: inline-flex; align-items: center; justify-content: center;
      width: 40px; height: 40px; border-radius: 8px; text-decoration: none;
      color: #111; font-size: 18px; transition: .18s;
    }
    .icon-link:hover { background: rgba(184,134,11,.1); color: #b8860b; }

    /* ── WRAPPER ── */
    .page-wrapper {
      width: 92%; max-width: 960px; margin: 36px auto 60px;
      display: flex; flex-direction: column; gap: 20px;
    }

    /* ── PAGE TITLE ── */
    .page-title {
      text-align: center; font-size: 32px; font-weight: 700; letter-spacing: .5px;
      color: #f8ce86; text-shadow: 0 2px 12px rgba(248,206,134,.35);
      margin-bottom: 4px;
    }
    .page-sub { text-align: center; font-size: 15px; color: rgba(255,255,255,.6); }

    /* ── ALERT MESSAGES ── */
    .alert-success, .alert-error {
      display: flex; align-items: center; gap: 10px;
      padding: 14px 18px; border-radius: 12px; font-size: 15px; font-weight: 600;
      animation: slideDown .3s ease;
    }
    .alert-success { background: rgba(76,175,80,.2); border: 1px solid #4caf50; color: #a5d6a7; }
    .alert-error   { background: rgba(244,67,54,.2);  border: 1px solid #f44336; color: #ef9a9a; }
    ...
    .order-card:hover { transform: translateY(-2px); box-shadow: 0 8px 32px rgba(248,206,134,.15); }
    ...
    .order-header-item { display: flex; flex-direction: column; gap: 2px; }
    .order-header-item .lbl { font-size: 11px; text-transform: uppercase; letter-spacing: .8px; color: rgba(255,255,255,.5); }
    .order-header-item .val { font-size: 14px; font-weight: 700; color: #f8ce86; }

    .order-header-actions { margin-left: auto; display: flex; align-items: center; gap: 8px; }
    ...
    .order-table th {
      ...
      background: rgba(248,206,134,.08); color: rgba(255,255,255,.55);
      font-size: 11px; text-transform: uppercase; letter-spacing: .8px;
      border-bottom: 1px solid rgba(248,206,134,.15);
    }
    .order-table td {
      padding: 10px 12px; border-bottom: 1px solid rgba(255,255,255,.06);
      color: rgba(255,255,255,.88); vertical-align: middle;
    }
    .order-table tr:last-child td { border-bottom: none; }
    .order-table td.price { color: #f8ce86; font-weight: 700; }
    ...
    .order-footer {
      ...
      font-size: 16px; font-weight: 700; color: #f8ce86;
    }
    ...
    .empty-state i { font-size: 56px; color: rgba(248,206,134,.3); display: block; margin-bottom: 16px; }
    .empty-state p { color: rgba(255,255,255,.5); font-size: 16px; margin-bottom: 20px; }
    .empty-state a {
      display: inline-flex; align-items: center; gap: 8px;
      padding: 12px 28px; background: linear-gradient(135deg,#f8ce86,#b8860b);
      color: #3b2f10; border-radius: 999px; text-decoration: none; font-weight: 700;
      transition: .2s;
    }
    .empty-state a:hover { transform: scale(1.03); }
    ...
    /* ── FOOTER ── */
    footer { text-align: center; padding: 20px; color: rgba(255,255,255,.4); font-size: 13px; margin-top: auto; }

    @media(max-width:640px) {
      .order-header { flex-direction: column; align-items: flex-start; }
      .order-header-actions { margin-left: 0; }

  ...
  <style>
    ...
    * {
    ...
    @keyframes pulse {
      ...
      60% {
        box-shadow: 0 0 0 22px rgba(76, 175, 80, 0);
      }
    ...
    <div class="search-bar">
      ...
      <div class="right">
        <a href="<?= $link_cart ?>" class="icon-link" title="Cart">
          ...
          <?php
          $uid_b = (int) $_SESSION['user_id'];
          $st_b = $conn->query("SELECT SUM(quantity) as total_qty FROM cart WHERE user_id = $uid_b");
          $total_cart_count = 0;
        ...
        <a href="<?= $link_profile ?>" class="icon-link" title="Profile"><i
            class="fas fa-user-circle user-icon"></i></a>
        <a href="<?= $link_logout ?>" class="icon-link" title="Logout" style="color:#111;"><i
          ...
          <?php foreach ($order_items as $item): ?>
            <div class="item-row">
              ...
              <div class="item-info">
                ...
                <div class="item-meta">SKU: <?= htmlspecialchars($item['product_id']) ?> · Qty:
                  <?= (int) $item['quantity'] ?>
                </div>
              </div>

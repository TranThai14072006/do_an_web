...
<body>
  ...
  <main class="content">
    <div class="page-header">
      <h1><i class="fas fa-file-import" style="margin-right:10px; opacity:.8;"></i>Import Management</h1>
      <a href="add_entry_form.php" class="btn btn-primary">+ New Entry Form</a>
    </div>
      ...
      <form method="GET" action="import_management.php" id="search-form">
        <div class="search-row">
          <div class="search-group wide">
            ...
            <input type="text" name="order" class="search-input" placeholder="Search by order number..."
              value="<?= htmlspecialchars($search_order) ?>">
          </div>
          <div class="search-group narrow">
            <label class="search-label">From Date</label>
            <input type="date" name="from" class="search-input" value="<?= htmlspecialchars($search_from) ?>">
          </div>
          <div class="search-group narrow">
            <label class="search-label">To Date</label>
            <input type="date" name="to" class="search-input" value="<?= htmlspecialchars($search_to) ?>">
          </div>
          <div class="search-group narrow">
          ...
          foreach ($rows as $row):
            ...
            <tr>
              ...
              <td>
                ...
                <?php else: ?>
                  <span style="color:#ccc;font-size:13px;" title="This form is completed and cannot be edited">🔒
                    Locked</span>
                <?php endif; ?>
              </td>
            </tr>
    ...
    <?php if ($total_pages > 1): ?>
      <div class="pagination-wrap">
        ...
        <div class="pagination">
          ...
          if ($start > 1): ?><button class="pagination-btn" onclick="goPage(1)">1</button><?php if ($start > 2): ?><span
                style="padding:0 4px;color:#999">…</span><?php endif; endif;
          for ($p = $start; $p <= $end; $p++): ?>
            <button class="pagination-btn <?= $p === $page ? 'active' : '' ?>"
              onclick="goPage(<?= $p ?>)"><?= $p ?></button>
          <?php endfor;
          if ($end < $total_pages): ?>    <?php if ($end < $total_pages - 1): ?><span
                style="padding:0 4px;color:#999">…</span><?php endif; ?><button class="pagination-btn"
              onclick="goPage(<?= $total_pages ?>"><?= $total_pages ?></button><?php endif; ?>
